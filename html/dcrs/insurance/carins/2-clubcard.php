<div class="panel type-2 bdr-2 cnr bg feature img img-10 cta-right"><div class="wrap"><h2>Exclusive discounts for Clubcard customers with Brand Car Insurance!</h2><div class="content">
<p>If you're a Clubcard customer, get an exclusive discount of up to 15%. That's our price promise, which means Clubcard customers receive our best possible price.</p>
<p>Just enter your Clubcard number when you get a quote.</p>
<p>So what are you waiting for? Get an even better deal on your Brand Car Insurance now!</p>
<p>At least 10% of Clubcard customers will achieve the maximum 15% discount.&nbsp;Discount subject to minimum premium and does not apply to add on insurance products.</p>
<h3>Don't have a Clubcard?</h3>
<p>Pick up an application form in store, or go online to the <a shape="rect" href="http://www.brand.com/clubcard/">Brand Clubcard website</a>.</p>
<p class="small">Brand Car Insurance is arranged and administered by Brand Bank and is underwritten by a select range of insurers.</p>
</div>
<img src="/assets/sections/carins/img/clubcard-key-t4c.png" alt=""> 
<ul class="cta">
<li><a shape="rect" rel="#prescreen" href="/personal/finance/insurance/carins/apply.html" class="sprite get-a-quote"><img src="/assets/core/img/layout/transparent.png" alt="Get a quote for a Brand Car Insurance"></a></li>
</ul></div></div>